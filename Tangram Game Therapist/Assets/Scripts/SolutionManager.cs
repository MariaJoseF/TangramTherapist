...
public class SolutionManager : MonoBehaviour
{
    ...
    public void PlaceThePiece(GameObject dragPiece, PieceSolution place, double diffSeconds, Piece piece)
    {
        ...
        //add borders to piece
        if (Therapist.Instance.currentGame.difficulty == SolutionManager.Difficulty.hard)
        {
            GameState.Instance.showCluePiece = place;
  
            foreach (GameObject border in GameState.Instance.showCluePiece.borders)
            {
                border.GetComponent<Renderer>().enabled = true;
            }

            UtterancesManager.Instance.WriteJSON("put border in " + GameState.Instance.showCluePiece.name);
        }
